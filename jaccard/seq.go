...
func main() {
	...
	showInfo := flag.Bool("showinfo", true, "Mostrar info del TARGET y TOP-1 desde el CSV original")
	...
	for i := 0; i < *reps; i++ {
		t0 := time.Now()
		top, coverage, meanVal, maxVal := jaccardAll(sets, targetID, *k)
	...
	if *out != "" {
		saveSummary(*out, "jaccard", "seq", *reps, rows, tMean, tStd, *k, lastCoverage, lastMean, lastMax)
	}
...
// ---------- Jaccard target vs todos ----------
func jaccardAll(sets map[string]map[string]struct{}, target string, K int) ([]Sim, int, float64, float64) {
	A := sets[target]
	scores := make([]Sim, 0, len(sets)-1)
	var sum float64
	var cnt int
	var maxv float64
	...
	for u, B := range sets {
		...
		scores = append(scores, Sim{User: u, Val: j})
		sum += j
		cnt++
		if j > maxv {
			maxv = j
